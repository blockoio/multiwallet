...
if not fwstatus then
    ...
    function TestReporter.recordError(name, msg)
      system.print(color.red .. "[FAIL] " .. name .. " - " .. msg .. color.reset)
    end

    function TestReporter.pass(name)
      system.print(color.green .. "[PASS] " .. name .. color.reset)
    end
end
...
abi = {
    ...
    register_view = function (funcname, ...)
    end,
...
function assertError(func, expectedErrMsg, errMsg)
  if not func or type(func) ~= 'function' then
    error("expression must be function type", 0)
  end

  local success, resultMsg = pcall(func)
  
  if success then error('expression must be errored', 0) end
  
  if expectedErrMsg then
    if string.match(resultMsg, expectedErrMsg) then
      return
    end
    error("'"..resultMsg.."' is not equal to "..expectedErrMsg, 0)
  elseif errMsg then
    error(errMsg, 0)
  else
    return
